...
class CommentObj extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (CommentPojo parentComment in tree) {
      render.add(commentBox(parentComment));
      if (parentComment.children != null) {
        for (CommentPojo child in parentComment.children!) {
          render.add(Padding(
            padding: const EdgeInsets.only(left: 15),
            child: commentBox(child),
          ));
          if (child.children != null) {
            for (CommentPojo grandKid in child.children!) {
              render.add(Padding(
                padding: const EdgeInsets.only(left:30),
                child: commentBox(grandKid),
              ));
            }
          }
        }
      }
      render.add(Divider(thickness: 4,));
    }
    return SafeArea(child: Column(children: render,));
  }

 Widget commentBox(CommentPojo comment) {
  return SafeArea(
    child: SizedBox(
      child: Row(
